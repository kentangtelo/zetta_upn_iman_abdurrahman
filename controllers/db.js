...
export const filterArrayKoleksiBookShelf = async (req, res) => {
  const genre = req.query.genreIndetifier;
...
export const listKoleksiGenreBookShelf = async (req, res) => {
  try {
...
export const listGenreSetiapBuku = async (req, res) => {
  try {
...
export const listGenreSetiapBukuUnwind = async (req, res) => {
  try {
    const bookShelf = await BookShelf.aggregate([
      { $unwind: "$books" },
      { $project: { title: "$books.judul", genre: "$books.genre" } },
      {
        $addFields: {
...
export const sortJudulBuku = async (req, res) => {
  const genre = req.query.genre;
    ...
    const bookShelf = await BookShelf.aggregate([
      ...
      { $match: { "books.genre": genre } },
      {
        $project: {
          ...
          genre: "$books.genre",
          publisher_writer: {
            $concat: ["$books.publisher", " - ", "$books.penulis"],
          },
        },
      },
      {
...
export const sortJudulBukuLookup = async (req, res) => {
  const genre = req.query.genre;
